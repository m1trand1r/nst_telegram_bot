...
async def process_help_command(message: types.Message):
    help_message = 'Бот работает на сервере без GPU ускорителя, поэтому итоговое изображение будет с измененным ' \
                   ...
                   'Для отмены переноса стиля отправьте сообщение с текстом cancel' \
                   '(не имеет разницы строчные или заглавные буквы)\n' \
                   'После подтверждения начала переноса стиля для начала нового переноса стиля необходимо дождаться ' \
                   'окончания переноса стиля.\n' \
                   'После запуска переноса стиля 10-20 секунд бот может не отвечать, так как трансфер нагружает' \
                   ' процессор на 100% в начале работы.'
